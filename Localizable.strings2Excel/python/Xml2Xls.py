...
def convertToMultipleFiles(fileDir, targetDir):
    ...
    ws = workbook.add_sheet("localizable")
    for _, dirnames, _ in os.walk(fileDir):
        valuesDirs = sorted([di for di in dirnames if (di.startswith("values") and os.path.exists(fileDir + "/" + di + "/" + "strings.xml"))])
        for index in range(len(valuesDirs)):
            dirname = valuesDirs[index]
            ws.write(0, index, dirname)
            for _, _, filenames in os.walk(fileDir+'/'+dirname):
                xmlFiles = [fi for fi in filenames if fi.endswith(".xml")]
                for xmlfile in xmlFiles:
                    (keys, values) = XmlFileUtil.getKeysAndValues(fileDir+'/'+dirname+'/' + xmlfile)
                    for keyindex in range(len(keys)):
                        key = keys[keyindex]
                        value = values[keyindex]
                        ws.write(keyindex+1, 0, key)
                        ws.write(keyindex+1, 1, value)
...
def startConvert(options):
    fileDir = "/Users/yyg/Documents/ukelink/fork/yyg/android/simbox-app/platforms/android/app/src/main/res"#options.fileDir
    targetDir = "examples/output"#options.targetDir
